...

{
  ...
  public function profile(User $employee,Award $award){
      ...
      return view('employee.profile',compact('custom_field_values','token','awards','messages12','training','tasks','employee','salary','profile','document_types','earning_salary_types','deduction_salary_types','employeeassets'));
  }
  ...
  public function update(EmployeeRequest $request, User $employee){
      ...
      if(!Entrust::can('edit_employee') || (!Entrust::can('manage_all_employee') && Entrust::can('manage_subordinate') && !Helper::isChild($employee->designation_id) ))
        ...
        $employee->paid_sick_leave = $request->input('paid_sick_leave')?$request->input('paid_sick_leave'):'0';
